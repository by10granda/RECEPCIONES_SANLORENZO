...
export function exportWarrantiesExcel(rows: Warranty[], filename = "garantias.xlsx") {
  const table = [["Código de garantía", "Cliente", "Documento", "Teléfono", "Factura", "Producto", "Código producto", "Serie", "Fecha recepción", "Estado", "Última actualización", "Vendedor"], ...rows.map((w) => [w.code, `${w.customerName} ${w.customerLastName}`, `${w.documentType} ${w.documentNumber}`, w.phone, w.invoiceNumber, w.productDescription, w.productCode, w.serialNumber, w.receptionDate, w.currentStatus, w.lastUpdate, w.sellerName])];
...
export function exportWarrantiesPdf(rows: Warranty[], filename = "garantias.pdf") {
  ...
  rows.forEach((w, idx) => {
    ...
    doc.text(`${idx + 1}. ${w.code} | ${w.customerName} ${w.customerLastName} | Factura: ${w.invoiceNumber} | ${w.productDescription} | Serie: ${w.serialNumber} | ${w.currentStatus}`, 12, y, { maxWidth: 272 });
    y += 8;
  });
  doc.save(filename);
}
